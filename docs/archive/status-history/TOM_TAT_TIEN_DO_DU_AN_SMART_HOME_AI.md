...
### 4.4. Logic tự động cắt tải đã được hiện thực nhưng đang khóa

Backend có logic thử nghiệm cho Demand Response, nhưng `autoLoadSheddingEnabled` mặc định là `false` và chưa được xem là kết quả phần cứng:
...
Nên mô tả hiện tại là: **đã hiện thực logic phần mềm nhưng đang khóa cho đến khi có phân loại tải thiết yếu, manual override, interlock vật lý, feedback verification và thử nghiệm tải an toàn**.
