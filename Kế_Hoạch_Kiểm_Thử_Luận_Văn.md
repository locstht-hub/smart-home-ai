...
3.  **Kiểm chứng các kịch bản vận hành thực tế HEMS (HEMS Operation Scenarios):** Thực nghiệm thuật toán sa thải phụ tải chủ động (Active Load-shedding) theo thứ tự ưu tiên khi hệ thống tiệm cận hoặc vượt mức giới hạn (Quota) đăng ký.
...
|---|---|---|---|---|
| **LAT-01** | Độ trễ lệnh bật/tắt thiết bị qua mạng nội bộ (LAN) | Bấm nút ON/OFF trên Mobile App (cùng Wi-Fi LAN) | Relay vật lý tác động tức thời. Trạng thái phản hồi trên App cập nhật khép kín. | Mục tiêu: $< 200 \text{ ms}$ |
...
| **SC-03** | Sa thải phụ tải chủ động (Active Load-shedding) | Công suất tức thời ($P_{actual}$) vượt ngưỡng hạn mức đăng ký ($P_{quota}$) | **Bước 1:** Gửi cảnh báo đỏ (Alert) lên Mobile App.<br>**Bước 2:** PLC tự động ngắt tải Priority 3 (Phòng bếp).<br>**Bước 3:** Nếu vẫn vượt quota, ngắt tiếp tải Priority 2 (Phòng khách). | Ngưỡng công suất quota ($kW$), Thời gian PLC kích hoạt lệnh ngắt bảo vệ ($ms$). |
...
|---|---|---|---|---|
| **SMS-01** | Kiểm thử cảnh báo khi đạt ngưỡng 80%, 90%, 100% quota tháng | Giả lập điện năng tiêu thụ tháng đạt các ngưỡng tương ứng trong DB | Ngưỡng 80% chỉ gửi thông báo App; Ngưỡng 90% và 100% gửi cả thông báo App và gọi API SMS Gateway gửi tin nhắn. | App notification + SMS trigger thành công |
