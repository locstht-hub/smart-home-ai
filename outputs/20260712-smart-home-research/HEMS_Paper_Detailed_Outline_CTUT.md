...
**Đối tượng nghiên cứu:** Mô hình HEMS quy mô phòng thí nghiệm, tích hợp MFM384, PLC Siemens S7-1200, cơ cấu đóng cắt, Web/App, quản lý Quota và dự báo phụ tải 24 giờ.
...
> Làm thế nào thiết kế và kiểm chứng một mô hình HEMS quy mô phòng thí nghiệm có khả năng đo đại lượng điện, xác nhận điều khiển phụ tải, dự báo 24 giờ và hỗ trợ quản lý điện năng theo Quota?
...
- **Hợp đồng hành vi phần mềm:** Có một phần; chỉ nêu hành vi có test tương ứng, không nói toàn bộ hệ thống đã kiểm thử thành công.
...
**THIẾT KẾ HỆ THỐNG GIÁM SÁT, QUẢN LÝ NĂNG LƯỢNG VÀ DỰ BÁO PHỤ TẢI CHO PHÒNG THÍ NGHIỆM ĐIỆN**

### Tên bài tiếng Anh

**Design of an Energy Monitoring, Management, and Load Forecasting System for an Electrical Engineering Laboratory**
...
1. **Bối cảnh:** Nhu cầu giám sát và quản lý điện năng trong phòng thí nghiệm Điện.
...
- **RQ3:** Điện năng tháng, Quota và kết quả dự báo được kết hợp như thế nào để cảnh báo và đề xuất sử dụng điện?

### Ba đóng góp dự kiến

1. Thiết kế chuỗi đo–điều khiển có phản hồi cho testbed phòng thí nghiệm.
2. Xây dựng cơ chế giám sát, Quota tháng, cảnh báo và khuyến nghị vận hành trên Web/App.
...
### Hình 1. Kiến trúc tổng thể của mô hình HEMS quy mô phòng thí nghiệm
...
> Hình 1. Kiến trúc testbed gồm chuỗi đo MFM384–S7-1200, chuỗi điều khiển relay/contactor có phản hồi và lớp Web/App hỗ trợ giám sát, dự báo và quản lý điện năng.
...
### Hình 3. Giao diện giám sát và quản lý điện năng

**Loại hình:** Ảnh chụp màn hình thật gồm ba panel:

- Dashboard V/I/P/E và trạng thái tải.
- Dự báo 24 giờ, điện năng tháng và trạng thái Quota.
- Điều khiển thiết bị kèm phản hồi thành công, từ chối, lỗi hoặc timeout.

Không dùng màn hình đăng nhập, ảnh trang trí hoặc chatbot chưa được đánh giá làm hình chính.
...
| Nhóm | Nội dung được phép báo cáo |
|---|---|
| Dữ liệu UCI | 17.521 dòng theo giờ; 15.321 mẫu có giám sát; phạm vi và checksum lấy từ kết quả chuẩn |
| Tiền xử lý | Tổng hợp giờ; causal fill tối đa 6 giờ; không dùng giá trị điền làm mục tiêu |
| Chia dữ liệu | Expanding rolling-origin; 3 fold; đúng thứ tự thời gian |
| Seed | 42, 3407 và 2026 |
| Đặc trưng | Chỉ liệt kê đặc trưng thực sự có trong pipeline |
| Random Forest | 25 cây; độ sâu tối đa 12 |
| XGBoost | 75 cây; độ sâu tối đa 5; learning rate 0,035 |
| Đầu ra | 24 mô hình/ước lượng trực tiếp cho h+1…h+24 |

Mọi giá trị trong bảng phải tiếp tục được lấy từ `canonical_results.json`, không sao chép từ tài liệu trạng thái cũ.

## 2.7. Kế hoạch thu thập dữ liệu tại phòng thí nghiệm Điện
...
### Bảng 4. Ma trận kịch bản và vị trí ghi kết quả thực nghiệm
...
- `backend`, `forecast`, `research`, `admin_audit` và `room_presentation` đã có suite đạt ở bằng chứng chuẩn hiện tại.
- `verify` và `frontend_contract` còn lỗi; vì vậy không tuyên bố toàn bộ repository hoặc toàn bộ trạng thái ứng dụng đã được kiểm thử thành công.
...
- Ghi rõ phạm vi suite đã đạt và suite còn lỗi.
...
### Hình 4. Diễn biến đại lượng điện và trạng thái phụ tải

**Loại hình:** Đồ thị thời gian tạo từ log thật.
...
Khi có log, điền cột “Kết quả” của Bảng 4 và thảo luận:
...
### Hình 5. Sai số dự báo theo chân trời
...
### Bảng 5. Kết quả benchmark dự báo trên tập test
...
- **Phần mềm:** Test mã nguồn không thay thế thử nghiệm điện; một số suite hiện chưa đạt.
...
- Hoàn thiện các suite phần mềm còn lỗi.
...
| Hình 3 | Dashboard, dự báo–Quota và phản hồi điều khiển | Screenshot thật | Có thể chụp sau khi làm sạch dữ liệu |
| Hình 4 | Diễn biến V/I/P/E và trạng thái tải | Đồ thị từ log thật | Chờ log phần cứng |
| Hình 5 | Sai số dự báo theo chân trời | Đồ thị tái lập | Có dữ liệu chuẩn |
...
| Bảng 3 | Cấu hình dữ liệu và mô hình dự báo | Có dữ liệu chuẩn |
| Bảng 4 | Kịch bản và kết quả thực nghiệm | Thiết kế có sẵn; kết quả chờ log |
| Bảng 5 | Kết quả benchmark dự báo | Có dữ liệu chuẩn |

**Tổng cộng:** 5 hình + 5 bảng = 10.

## Kiểm tra trước khi viết bản hoàn chỉnh

- [x] Đúng bốn phần chính của bài báo.
- [x] Có đúng ba câu hỏi nghiên cứu.
- [x] Tổng hình và bảng không vượt quá 10.
...
- [x] Phòng thí nghiệm được định vị là testbed thực nghiệm.
...
- [ ] Chụp App cho Hình 3 bằng dữ liệu đúng và che thông tin nhạy cảm.
- [ ] Thực hiện kịch bản Bảng 4 để điền kết quả và tạo Hình 4.
- [ ] Xác minh DOI và định dạng toàn bộ tài liệu tham khảo.
- [ ] Chỉ sau đó mới viết toàn văn và xuất DOCX/PDF theo mẫu CTUT.
